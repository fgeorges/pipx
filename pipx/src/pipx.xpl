<p:library xmlns:p="http://www.w3.org/ns/xproc"
           ...
           xmlns:impl="http://pipx.org/ns/pipx/impl"
           exclude-inline-prefixes="xs pkg pipx impl"
           version="1.0"
           pkg:import-uri="http://pipx.org/ns/pipx.xpl">

   <p:declare-step type="pipx:error" name="this">
      <p:documentation xmlns="http://www.w3.org/1999/xhtml">
         <p>Throws an error, with a proper title, message, and other infos.</p>
         <p>This is the central piece in the
            error handling machanism, still to define precisely.</p>
         <p><b>TODO</b>: Error handling machanism, still to define precisely!</p>
         <p><b>TODO</b>: Enable the ability to
            give an input to p:error, through an input to this step. Maybe we should have either the
            option <code>message</code> or the port <code>message</code> (a template in both
            cases).</p>
      </p:documentation>
      <p:option name="code" required="true">
         <p:documentation xmlns="http://www.w3.org/1999/xhtml">
            <p>The error code, following the same requirements as the option
               <code>code</code> of <code>p:error</code>.</p>
         </p:documentation>
      </p:option>
      <p:option name="code-prefix" required="false">
         <p:documentation xmlns="http://www.w3.org/1999/xhtml">
            <p>The error code prefix, following the same requirements as the option
               <code>code-prefix</code> of <code>p:error</code>.</p>
         </p:documentation>
      </p:option>
      <p:option name="code-namespace" required="false">
         <p:documentation xmlns="http://www.w3.org/1999/xhtml">
            <p>The error code namespace, following the same requirements as the option
               <code>code-namespace</code> of <code>p:error</code>.</p>
         </p:documentation>
      </p:option>
      ...
      <p:template name="title-tpl">
         ...
         <p:input port="template">
            <p:inline><pipx:title>{ $title }</pipx:title></p:inline>
         </p:input>
      ...
      <p:template name="message-tpl">
         ...
         <p:input port="template">
            <p:inline><pipx:message>{ $message }</pipx:message></p:inline>
         </p:input>
   ...
   <p:declare-step type="pipx:parameter" name="this">
      <p:documentation xmlns="http://www.w3.org/1999/xhtml">
         ...
         <p>The parameters are passed on the
            primary port <code>parameters</code>, of kind <code>parameters</code>. The name of the
            parameter to extract is in the option <code>name</code>. If the option
               <code>required</code> is <code>true</code> (default is <code>false</code>), the step
            throws the error <code>pipx:no-parameter</code> if there is no such parameter or if its
            value is an empty string.</p>
         <p>The returned document, on the port
               <code>result</code>, contains a root element <code>param</code>, with an attribute
               <code>name</code>. The attribute value is the name of the parameyer (the value of the
            option <code>name</code>). The value of the element, is the value of the parameter. If
            the parameter does not exist (and the value of the option <code>required</code> is
               <code>false</code>), the element value can be empty.</p>
         <p>If the value of the parameter
               <code>required</code> is not a valid lexical <code>xs:boolean</code>, the step throws
            the error <code>pipx:invalid-option</code>. The valid lexical values are
               <code>false</code>, <code>true</code>, <code>0</code> and <code>1</code>, with
            optional heading and trailing whitespaces.</p>
      ...
      <p:choose>
         <p:when test="not($required castable as xs:boolean)">
            <impl:error code="invalid-option"
                        title="Invalid option to pipx:parameter."
                        message="The option $required as an invalid value: '{ $value }'.">
         ...
         <p:otherwise>
            <impl:error code="no-parameter"
                        title="Required parameter does not exist."
                        message="The parameter '{ $name }' is required and has no value.">
               <p:with-param name="name" select="$param-name"/>
            </impl:error>
         </p:otherwise>
      </p:choose>
